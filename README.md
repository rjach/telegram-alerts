...
3. Find the chat id and send a test:
   ...
   ```bash
   TELEGRAM_BOT_TOKEN=123:abc npx telegram-alerts setup --product "Dingcut"
   ```
...
The CLI also reads `./.env`, so from a project directory `npx telegram-alerts setup` is enough.
...
```ts
// lib/alerts.ts
import { createTelegramAlerts } from 'telegram-alerts';
